...
#Scheduling function
def schedulingFunction(temp, decay_rate):
    return temp * decay_rate
...
def simulatedAnnealing(initBoard, decayRate, T_Threshold):
    ...
    while (T >= T_Threshold):
        T = schedulingFunction(T, decayRate)
        if (T < T_Threshold or current.h == 0):
            return current

        nextblist = getSuccessorStates(current)
        n = random.random() * len(nextblist) - 1
        nextb = nextblist[int(n)]

        E = numAttackingQueens(current) - numAttackingQueens(nextb)
        if (E > 0):
            current = nextb
        else:
            if (random.random() > m.exp(E / T)):
                current = nextb
...
board1.printBoard()
print("\n\n#############################################\n\n")
board2 = simulatedAnnealing(board1, 0.7, 5)
board2.printBoard()
